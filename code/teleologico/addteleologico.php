...
<head>
    ...
    <style>
        ...
        .btn-ver-mas {
            background: none; border: none; color: #412fd1;
...
<div class="page-wrapper">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-3">
        <h1 class="page-title mb-0"><i class="fas fa-chalkboard-teacher me-2"></i>Componente Teleológico</h1>
        <a href="addteleologico1.php" class="btn btn-success d-flex align-items-center gap-2 rounded-pill shadow-sm px-4">
            <i class="fas fa-plus"></i><span>Agregar Componente Teleológico</span>
        </a>
    </div>
    <?php
    ...
    while ($row = mysqli_fetch_array($result)) {
        $id_ct = (int)$row['id_ct'];
        $fe = (!empty($row['fecha_edit_ct']) && $row['fecha_edit_ct'] !== '0000-00-00 00:00:00') ? $row['fecha_edit_ct'] : '—';
        echo '
        <tr>
            ...
            <td class="cell-truncate"><div class="cell-text">' . htmlspecialchars($row['mision_ct']) . '</div><button class="btn-ver-mas" onclick="toggleVerMas(this)">Ver más ▾</button></td>
            <td class="cell-truncate"><div class="cell-text">' . htmlspecialchars($row['vision_ct']) . '</div><button class="btn-ver-mas" onclick="toggleVerMas(this)">Ver más ▾</button></td>
            <td class="cell-truncate"><div class="cell-text">' . htmlspecialchars($row['per_egr_ct']) . '</div><button class="btn-ver-mas" onclick="toggleVerMas(this)">Ver más ▾</button></td>
            <td class="cell-truncate"><div class="cell-text">' . htmlspecialchars($row['obj_ins_ct']) . '</div><button class="btn-ver-mas" onclick="toggleVerMas(this)">Ver más ▾</button></td>
            ...
            <td class="text-center"><a href="addteleologicoedit.php?id_ct=' . $id_ct . '" class="btn-action-icon btn-edit" title="Editar"><i class="fas fa-edit"></i></a></td>
            <td class="fecha-cell">' . ($row['fecha_alta_ct'] ?? '—') . '</td>
            <td class="fecha-cell">' . $fe . '</td>
            <td class="text-center"><button onclick="eliminarRegistro(' . $id_ct . ')" class="btn-action-icon btn-delete" title="Eliminar"><i class="fas fa-trash"></i></button></td>
        </tr>';
    }
